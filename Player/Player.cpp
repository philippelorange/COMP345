...
void Player::reinforce() {
    ...
    if (*type == 1) {
        ...
        cout << this->get_player_name() << " has placed " << *num_armies << " armies at "
             << strongestCountry->get_name() << endl;
        ...
        cout << this->get_player_name() << " has placed " << *num_armies << " armies at " << weakestCountry->get_name()
             << endl;
        for (auto& owned_country : *owned_countries) {
            cout << owned_country->get_name() << " has " << owned_country->get_nb_armies() << " armies" << endl;
        }
        ...
        for (int i = 0; i < *num_armies; i++) {
            ...
            while (selection < 1 || selection > this->get_player_owned_countries()->size()) {
                ...
                for (int k = 0; k < this->get_player_owned_countries()->size(); k++) {
                    cout << "\t \t (" << (k + 1) << ") " << this->get_player_owned_countries()->at(k)->get_name()
                         << endl;
                }
    ...
    if (*type == 1) {
        ...
        while (true) {
                ...
                if (*defending_player->strategy->execute_strategy() == 3) {
                    ...
                    do {
                        cout << "How many dices does " << defending_player->get_player_name()
                             << " want to to defend with? (between 1 and " << max_number_of_dices_defense << ")"
                             << endl;
            ...
            if (attacker_won) {
                ...
                cout << *player_name << " , select the number of troops you want to move to "
                     << attack_target->get_name()
                     << "(between 1 and " << (armies_in_attacking_country - 1) << ")" << endl;
                cout << *player_name << " select to move " << (armies_in_attacking_country - 1) << " armies" << endl;
                int armies_moved_to_conquered_country = armies_in_attacking_country - 1;
        ...
        while (player_wants_to_attack) {
            ...
            while (selection < 1 || selection > valid_sources->size()) {
                cout << this->get_player_name()
                     << ", please select a country to attack from among the following countries:"
                     << endl;
            ...
            while (player_wishes_to_attack && (!battle_is_over)) {
                ...
                if (*defending_player->strategy->execute_strategy() == 3) {
                    do {
                        cout << "How many dices do you want to attack with? (between 1 and "
                             << max_number_of_dices_attack
                             << ")"
                             << endl;
        ...
        if (can_player_fortify == "You can fortify!") {
            ...
            for (int i = 0; i < strongest_target->get_adjacent_countries()->size(); i++) {
                ...
                if (can_player_fortify_source_and_target == "Can fortify") {
                    ...
                    cout << *this->player_name << " won't fortify, neighbour country " << source_country->get_name()
                         << " has "
                         << source_country->get_nb_armies() << " armies while strongest country has "
                         << target_country->get_nb_armies() << " armies" << endl;
                }
            }
        }
    } else if (*type == 2) {
                    ...
                    while (true) {
                        ...
                        if (can_player_fortify_source_and_target == "Can fortify") {
                            ...
                            while (true) {
                                ...
                                if (nbr_of_armies_moved > 0 &&
                                    nbr_of_armies_moved < source_country_ptr->get_nb_armies()) {
